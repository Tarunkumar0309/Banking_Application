...
export class OperateTransactionsComponent {
  ...
  onSubmit() {
    ...
    if (v.type === 'DEPOSIT') {
      ...
      this.txnService.transfer({ from_account_id: v.account_id, to_account_id: v.to_account_id!, amount: v.amount }).subscribe(this.handleResp());
    }
  }

  private handleResp() {
    return {
      next: () => { this.loading.set(false); this.message.set('Transaction successful'); },
      error: (e: any) => { this.loading.set(false); this.message.set(e?.error?.error || 'Transaction failed'); }
    } as any;
  }
}
