    ...
    <div class="grid" *ngIf="accounts().length; else empty">
      <div class="card" *ngFor="let a of accounts()">
        ...
        <div class="actions">
          ...
          <button class="danger" *ngIf="isAdmin" (click)="close(a.account_id)">Close</button>
        </div>
      </div>
    </div>
    <ng-template #empty>
...
export class AccountsListComponent {
  ...
  constructor() {
    this.load();
    try { const t = localStorage.getItem('auth_token'); this.isAdmin = !!t && JSON.parse(atob(t.split('.')[1])).role === 'ADMIN'; } catch { this.isAdmin = false; }
  }
